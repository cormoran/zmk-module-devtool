...
static int test_rpc_log_streaming(void) {
     ...
     * work item poll, drain and raise the notification (interval is 5ms in the
     * test config). Do not drain the log with log_process() here: by the time
     * this test runs the mock keyboard is generating log messages
     * continuously, so a manual drain never returns. */
    for (int i = 0; i < 40 && !stream_saw_marker; i++) {
        k_sleep(K_MSEC(1));
    }
...
static int devtool_rpc_test_init(void) {
     ...
     * already started (POST_KERNEL) by the time this APPLICATION-level init
     * runs, so it can be driven directly from here. */
    ret = test_rpc_log_streaming();
    if (ret < 0) {
        return ret;
    }
#endif
